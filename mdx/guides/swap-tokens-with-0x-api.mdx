...
A description of each field is outlined in the [API reference](../api/index.mdx#response).
...
* If `sellToken` is an ERC20 token (i.e., not `ETH`), the taker will need to grant the `allowanceTarget` contract an allowance first.
...
For a full list of valid sources, see: ....
